...
def fusion_fools_gold(
    global_model: torch.nn.Module,
    model_updates: Dict[int, torch.nn.Module],
) -> Dict[str, Any]:
    size_clients = len(model_updates)
    global_params = global_model.state_dict()
    global_weight = torch.cat(
        [v.reshape(-1).float() for v in global_params.values()]
    ).unsqueeze(0)

    _updates = []
    for _model in model_updates.values():
        _parameters = _model.state_dict()
        model_weight = torch.cat(
            [v.reshape(-1).float() for v in _parameters.values()]
        ).unsqueeze(0)
        _update = model_weight - global_weight
        _updates.append(_update)

    update_cat = torch.cat(_updates, dim=0)
    update_norm = torch.nn.functional.normalize(update_cat, dim=1)
    cosine_similarity = torch.mm(update_norm, update_norm.T) - torch.eye(
        size_clients, device=update_cat.device
    )
    max_cosine_similarity, _ = torch.max(cosine_similarity, axis=1)

    for i in range(size_clients):
        for j in range(size_clients):
            if i != j and max_cosine_similarity[i] < max_cosine_similarity[j]:
                cosine_similarity[i][j] = cosine_similarity[i][j] * (
                    max_cosine_similarity[i] / max_cosine_similarity[j]
                    if max_cosine_similarity[j] > 0
                    else 0
                )
    max_cosine_similarity, _ = torch.max(cosine_similarity, axis=1)
    wv = torch.clamp(1 - max_cosine_similarity, 0, 1)
    wv = wv / torch.max(wv)
    wv[wv == 1] = 0.99
    ...
    model_weight_list = [
        global_weight + wv[i] * update_cat[i] for i in range(size_clients) if wv[i] != 0
    ]
    fools_gold_weight = torch.mean(torch.stack(model_weight_list), dim=0)

    return fools_gold_weight


def fusion_dual_defense_enhance(
    global_model: torch.nn.Module,
    model_updates: Dict[int, torch.nn.Module],
    data_size: Dict[int, int],
    similarity_threshold: float = None,
...
def fusion_dual_defense(
    ...
    # 7) send to client for decryption
    fused_params = [param.decrypt() for param in fused_enc_params]
    fused_model = load_model_from_parameters(fused_params, global_model)
